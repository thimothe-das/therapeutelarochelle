...
export function HomePricingSection() {
  const {data, error} = useSWR(`${process.env.NEXT_PUBLIC_API_URL}/pages?slug=pricing`, fetcher)
    ...
    <section className="py-16 px-4 bg-gray-50" id="prestation">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-10">
          <h2 className="text-3xl font-bold mb-4 text-gray-800">{data?.acf.title}</h2>
          ...
          {pricingPlans?.map((plan) => (
            <div
              ...
              className={cn(
                ...
                plan.popular ? "border-teal-200 shadow-teal-100" : "border-gray-200",
              )}
            >
              {plan.popular && (
                <div className="absolute right-0 top-0 z-10">
                  <div className="bg-teal-600 text-white text-xs font-medium px-3 py-1 rounded-bl-lg">Populaire</div>
                </div>
              )}
              <div className="p-5">
                ...
                <div className="mt-4 flex items-end gap-1">
                  ...
                  <span className="text-sm font-medium text-gray-500">/ séance</span>
                </div>
        ...
        {data?.acf.action_button_link &&
         <div className="text-center">
          <Button asChild variant="ghost" className="group text-teal-700 hover:text-teal-800">
            <Link href={data?.acf.action_button_link} className="flex items-center gap-2">
